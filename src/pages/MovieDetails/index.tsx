    ...
    return (
      <Container>
          ...
          <MovieBody>
            ...
            <RowInformation>
              ...
              <DetailBox>
                ...
                <Text>
                  {this.convertMinutesToHours(this.state.movie.runtime)}
                </Text>
              </DetailBox>
              <DetailBox>
              ...
              <DetailBox>
                ...
                <Text>
                  ${this.state.movie.revenue - this.state.movie.budget},00
                </Text>
              </DetailBox>
            </RowInformation>
            <Row>
              {this.state.movie.genres &&
                this.state.movie.genres.map((genre: any) => (
                  <Chips key={genre.id}>{genre.name}</Chips>
                ))}
            </Row>
            <RoundProgress>
              {this.state.movie.vote_average &&
                this.handlePrepareRoundeProgress(this.state.movie.vote_average)}
            </RoundProgress>
          </MovieBody>
          <Poster
            src={
        ...
        this.state.movie.videos.results.length > 0 ? (
          <Video
            src={this.prepareVide(this.state.movie.videos.results[0].key)}
          ></Video>
        ) : (
          ""
        )}
